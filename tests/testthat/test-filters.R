...
test_that("test filter functions", {
  y1 <- make_sequence(B2721, "all")
  y2 <- make_sequence(B2721, c("ch1", "CHR4"))
  y3 <- make_sequence(B2721, arg = y1$mrk.names[1:10])
  y4 <- make_sequence(B2721, 1:10)
  ...
  s2 <- make_sequence(dat.filt, "Chr1")
