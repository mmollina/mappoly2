...
#' @export
calc_genoprob <- function(input.seq,
                          phase.conf = "all",
                          rf = NULL,
                          error = 0.0,
                          verbose = FALSE,
                          tol = 10e-4)
{
  ...
  genoprob <- vector("list", length(phase.conf))
  ...
  if (detect_info_par(input.seq) == "both"){
    for(i in 1: length(phase.conf)){
      ...
      pedigree <- matrix(rep(c(1,
                         ...
                         byrow = TRUE)
      genoprob <- calc_genoprob_biallelic(PH = PH,
                                               G = g,
                                               pedigree = pedigree,
    ...
    for(i in phase.conf){
      cat("   Conf.", i,":")
      genoprob <- calc_genoprob_biallelic_single(PH = PH,
                                                      G = g,
                                                      rf = input.seq$phases[[i]]$rf,
                                                      err = error)
      return(genoprob)
      cat("\n")
    }
