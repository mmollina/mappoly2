...
struct rf_two_point_parallel : public Worker {
  ...
  const std::vector<int> dose_p1;
  const std::vector<int> dose_p2;
  ...
  : mrk_pairs(mrk_pairs), geno(geno), dose_p1(dose_p1), dose_p2(dose_p2),
    ...
    count_matrix_rownames(count_matrix_rownames), count_matrix_number(count_matrix_number),
    count_matrix_pos(count_matrix_pos), count_matrix_length(count_matrix_length),
    ploidy_p1(ploidy_p1), ploidy_p2(ploidy_p2), tol(tol), n_ind(n_ind), out(out) {}
  ...
  void operator()(std::size_t begin, std::size_t end) {
    for (std::size_t k = begin; k < end; k++) {
      int id = (ploidy_p1+1) *
        (ploidy_p1+1) *
        (ploidy_p2+1) *
        dose_p2[mrk_pairs(1,k)] +
        (ploidy_p1+1) *
        (ploidy_p1+1) *
        dose_p2[mrk_pairs(0,k)] +
        (ploidy_p1+1) *
        dose_p1[mrk_pairs(1,k)] +
        dose_p1[mrk_pairs(0,k)] +
        1;
          ...
          for(int auau=0; auau < count_matrix_number[(id-1)]; auau++) //MODIFIED
            {
              ...
              for(int i = 0; i < gen_1.size(); i++)
                temp += log10(Tr[gen_1[i] + (gen_2[i] * (offspring_ploidy+2))]);
                                                 ...
                                                 SEXP tol_R)
{
  ...
  std::vector<int> dose_p1 = as<std::vector<int>>(dose_p1_R);
  std::vector<int> dose_p2 = as<std::vector<int>>(dose_p2_R);
  ...
  rf_two_point_parallel RF_two_point_parallel(mrk_pairs, geno, dose_p1, dose_p2, count_vector,
                                              ...
                                              count_matrix_length, ploidy_p1, ploidy_p2,
                                              tol, n_ind, out);
