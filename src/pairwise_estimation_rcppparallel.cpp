...
struct rf_two_point_parallel : public Worker {
  ...
  const std::vector<int> d_p1;
  const std::vector<int> d_p2;
  ...
  rf_two_point_parallel(const NumericMatrix mrk_pairs,
                        ...
                        const int n_ind,
                        NumericMatrix out)
  : mrk_pairs(mrk_pairs), geno(geno), d_p1(d_p1), d_p2(d_p2), count_vector(count_vector), count_matrix_phases(count_matrix_phases), count_matrix_rownames(count_matrix_rownames), count_matrix_number(count_matrix_number), count_matrix_pos(count_matrix_pos), count_matrix_length(count_matrix_length), m(m), tol(tol), n_ind(n_ind), out(out) {}

  // function call operator that work for the specified range (begin/end)
  void operator()(std::size_t begin, std::size_t end) {
    for (std::size_t k = begin; k < end; k++) {
      int id = (m+1)*(m+1)*(m+1)*d_p2[mrk_pairs(1,k)]+(m+1)*(m+1)*d_p2[mrk_pairs(0,k)]+(m+1)*d_p1[mrk_pairs(1,k)]+d_p1[mrk_pairs(0,k)]+1;
          ...
          for(int i=0; i < count_matrix_number[(id-1)]; i++) //MODIFIED
            {
              ...
              // fx = twopt_likelihood_dosage_rcpp(x, m, n_ind, d_p1[mrk_pairs(0,k)], d_p2[mrk_pairs(0,k)], dk, dk1, gen_1, gen_2, count_mat, z.size());
              fv = fx;
              fw = fx;
              tol3 = tol / 3.;
              ...
              for(;;) {
                ...
                // fu = twopt_likelihood_dosage_rcpp(u, m, n_ind, d_p1[mrk_pairs(0,k)], d_p2[mrk_pairs(0,k)], dk, dk1, gen_1, gen_2, count_mat, z.size());
              ...
              // res2 = twopt_likelihood_dosage_rcpp(x, m, n_ind, d_p1[mrk_pairs(0,k)], d_p2[mrk_pairs(0,k)], dk, dk1, gen_1, gen_2, count_mat, z.size());
                                                 ...
                                                 SEXP threads_R)
{
  ...
  std::vector<int> d_p1 = as<std::vector<int>>(d_p1_R);
  std::vector<int> d_p2 = as<std::vector<int>>(d_p2_R);
  ...
  rf_two_point_parallel RF_two_point_parallel(mrk_pairs, geno, d_p1, d_p2, count_vector,
