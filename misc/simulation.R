...
test_simulate <- function(ploidy.p1,
                          ...
                          seed = 123,
                          random = FALSE){
  snp_id <- P1 <- hp <- P2 <- hq <- genome_pos <- chrom <- ref <- alt <- dat <- geno <- NULL
  PH <- vector("list", n.chrom)
  for(i in 1:n.chrom){
    h <- sim_homologous(ploidy.p1, ploidy.p2, n.mrk, seed = seed * i)
    rf.vec <- mappoly::mf_h(diff(seq(0, map.length, length.out = n.mrk)))
    dat <- as.matrix(t(sim_cross_two_informative_parents(ploidy1 = ploidy.p1,
                                                         ploidy2 = ploidy.p2,
                                                         rf.vec = rf.vec,
                                                         hom.allele.p = h$hom.allele.p,
                                                         hom.allele.q = h$hom.allele.q,
                                                         n.mrk = n.mrk, n.ind = n.ind)$data.sim.two.parents))
    rownames(dat) <- paste0("Ch_", i, "_", rownames(dat))
    snp_id = c(snp_id, rownames(dat))
    P1 = c(P1, h$p)
    P2 = c(P2, h$q)
    chrom = c(chrom, rep(paste0("Ch",i), n.mrk))
    genome_pos = c(genome_pos, 1:n.mrk)
    ref = c(ref, sample(B2721$ref)[1:n.mrk])
    alt = c(alt, sample(B2721$alt)[1:n.mrk])
    p1 = mappoly::ph_list_to_matrix(h$hom.allele.p, ploidy.p1)
    p2 = mappoly::ph_list_to_matrix(h$hom.allele.q, ploidy.p2)
    rownames(p1) <- rownames(p2) <-  rownames(dat)
    PH[[i]] <- list(p1 = p1, p2 = p2)
    geno = rbind(geno, dat)
  }
  id<-sample(length(geno),(length(geno) * miss.perc/100))
  geno[id] <- NA
  x<-data.frame( snp_id = snp_id,
                 P1 = P1,
                 P2 = P2,
                 chrom = chrom,
