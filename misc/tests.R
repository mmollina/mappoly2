...
input.mat<-rf_list_to_matrix(input.twopt = tpt5)
plot(input.mat, ord = mrk.id)
sf5<-rf_snp_filter(tpt5, probs = c(0.1,0.9))
plot(input.mat, ord = intersect(mrk.id, sf5$mrk.names), fact = 5)

y7 <- make_seq(input.data, c("ch1", "CHR4", "ch10"))
tpt7 <- est_pairwise_rf(y7, ncpus = 7)
m7 <- rf_list_to_matrix(tpt7)
gr<-group_mappoly(input.mat = m7, expected.groups = 20, comp.mat = T)
